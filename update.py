...
def main():
    ...
    try:
        ...
        # ─── STAGE 1: Stats (only updates active campaigns, preserves others) ──
        ...
        # ─── STAGE 2: Sequences (only for active campaigns, append new ones) ──
        log("=== STAGE 2: SmartLead Sequences ===")
        sl_sequences = fetch_smartlead_sequences()
        # Don't clear — just append new sequences. Dedupe not needed since
        # sequences rarely change and duplicates are harmless for reference.
        append_rows(sheets, "Raw SmartLead Sequences", sl_sequences)
        log(f"  Appended {len(sl_sequences)} sequence rows")

        # ─── STAGE 3: Replies (always append, never clear) ─────────
        log("=== STAGE 3: SmartLead Replies ===")
